(function($) {
    $.fn.khylighted = function( options ) {
        var settings = $.extend({
            text: 0,
            css: '',
            caseSensitive: true,
		...
		return this.each( function() {
			...
			if(str_sensitive === false) str_filter = str_filter.toLowerCase();

	        $(str_split).each(function(key, value) {
	        	if(str_sensitive === false) value_filter = value.toLowerCase();
	        	else value_filter = value;

				if(str_filter == value_filter) tmp[key] = "<span class='"+css_style+"'>"+value+"</span>";
				else tmp[key] = value;
	        });
